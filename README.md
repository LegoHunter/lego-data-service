...
- Draft creation may omit `unitPrice`; an unpriced non-fixed `DRAFT` is eligible for Pricing Plane onboarding and reports `INITIAL_PRICE_PENDING` until Pricing Plane applies a positive initial price. Unpriced active or fixed listings still report `MISSING_UNIT_PRICE`.
